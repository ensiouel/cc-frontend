      ...
      <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
          ...
          <span @click="$router.push({name: 'signup'})"
                class="cursor-pointer font-medium text-indigo-600 hover:text-indigo-500">Sign Up</span>
        </p>
      </div>
      <div class="mt-8 space-y-6">
        <div class="-space-y-px rounded-md grid gap-2 shadow-sm">
          <div>
            ...
            <input v-model="name" id="username" name="username" type="text" autocomplete="username" required
...
export default {
  ...
  methods: {
    async submit() {
      this.error = null
      try {
        await this.authApi.signin({name: this.name, password: this.password})
